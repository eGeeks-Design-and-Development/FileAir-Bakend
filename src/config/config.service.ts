...
export class ConfigService extends EventEmitter {
  ...
  private async loadYamlConfig() {
    ...
    try {
      this.yamlConfig = configFile
        ? (yamlParse(configFile) as Partial<YamlConfig>)
      ...
      if (this.yamlConfig) {
        for (const configVariable of this.configVariables) {
          ...
          // @ts-ignore – dynamic indexing from YAML object
      ...
      this.yamlConfig = {} as Partial<YamlConfig>;
    }
  }
  ...
  private async migrateInitUser(): Promise<void> {
    ...
    if (!initUser?.enabled) {
      this.logger.log(
        'Initial user migration disabled or missing in config.yaml.',
      );
      return;
    }
    ...
    if (userCount >= 1) {
      this.logger.log(
        'Skip initial user creation. Admin user already exists.',
      );
      return;
    }
